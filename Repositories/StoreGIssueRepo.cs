...
namespace RMS.Repositories
{
    public class StoreGIssueRepo : IStoreGIssue
    {
        ...
        public StoreGIssueRepo(RmsDbContext context, IHttpContextAccessor httpContextAccessor)
        {
            _context = context;
        ...
        public async Task<StoreGIssueMaster> GetById(int id)
        {
            var data = from model in _context.StoreGIssueMasters.Where(x => x.GIMId == id).Include(x => x.StoreGIssueDetails)
                       select new StoreGIssueMaster
                       {
                           ...
                           HRDId= model.HRDId,
                           HRDepartName = model.HRDepart.HRDName,
                           GIMRemarks = string.IsNullOrEmpty(model.GIMRemarks)?string.Empty: model.GIMRemarks,
                           CUser = model.CUser,
