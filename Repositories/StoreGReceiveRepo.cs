...
namespace RMS.Repositories
{
    public class StoreGReceiveRepo : IStoreGReceive
    {
        ...
        public StoreGReceiveRepo(RmsDbContext context, IHttpContextAccessor httpContextAccessor)
        {
            _context = context;
        ...
        public async Task<List<StoreGReceiveMaster>> GetAll()
        {
            ...
            foreach (var item in data)
            {
                result.Add(new StoreGReceiveMaster
                {
                    ...
                    StoreSuppliersName = item.StoreSuppliers.SSName,
                    GRMRemarks = string.IsNullOrEmpty(item.GRMRemarks)?string.Empty: item.GRMRemarks,
                    CUser = item.CUser
                });
            }
            return result;
        ...
        public async Task<StoreGReceiveMaster> GetById(int id)
        {
            var data = from master in _context.StoreGReceiveMasters.Where(x => x.GRMId == id).Include(x => x.StoreGReceiveDetails)
                       select new StoreGReceiveMaster
                       {
                           ...
                           GRMRemarks = string.IsNullOrEmpty(master.GRMRemarks) ? string.Empty : master.GRMRemarks,
                           CUser = master.CUser,
